...
#+BEGIN_SRC emacs-lisp
  (defun my/ac-c-header-init ()
    ...
    (add-to-list 'achead:include-directories '"/usr/lib/gcc/x86_64-pc-linux-gnu/13.1.1/include")
    (add-to-list 'achead:include-directories '"/usr/lib/gcc/x86_64-pc-linux-gnu/13.1.1/include-fixed")
    )
...
** Dired							      :issue:
- Issue: use-packages =tramp= to be loaded?
...
- Set switches for dired to =-lha= for long format (use-packaged by =dired=), human readable file size, show all files and list directories first
#+BEGIN_SRC emacs-lisp
